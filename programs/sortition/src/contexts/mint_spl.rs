...
use crate::states::GovernancePool;
use anchor_spl::{
...
pub struct InitializeToken<'info> {
    ...
    #[account(
        ...
        mint::authority = mint,
        seeds = [GovernancePool::SPL_PREFIX_SEED, governance_pool.key().as_ref(), params.symbol.as_bytes()], 
        bump
    )]
...
pub fn initialize_token(
    ...
    let governance_pool_key = governance_pool.key();
    let payer = &ctx.accounts.payer.key();
    governance_pool.spl_symbol = params.symbol.clone();

    let metadata_seeds = &[
        GovernancePool::SPL_PREFIX_SEED,
        governance_pool_key.as_ref(),
    ...
    initialize_mint(cpi_context, params.decimals, &payer.key(), Some(&governance_pool.key()))?;
...
pub fn mint_tokens(ctx: Context<MintTokens>, amount_to_treasury: u64, amount_to_citizen: u64) -> Result<()> {
    ...
    if governance_pool.spl_minted as u64 + total_mint_amount > governance_pool.total_spl_token_supply as u64 {
    ...
    governance_pool.spl_minted = governance_pool.spl_minted.checked_add(total_mint_amount as u32)
        .ok_or(GovernanceError::Overflow)?;

    governance_pool.total_sbt_token_supply += 1;
    ...
    emit!(MintEvent {
        ...
        total_minted: governance_pool.spl_minted,
    });
