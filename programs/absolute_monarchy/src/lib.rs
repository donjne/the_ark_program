...
pub mod absolute_monarchy {
    ...
    pub fn initialize_absolute_monarchy(ctx: Context<InitializeKingdom>, kingdom_name: String, monarch_name: String, divine_mandate: String) -> Result<()> {
        contexts::init_state::initialize_kingdom(ctx, kingdom_name, monarch_name, divine_mandate)
    }

    pub fn abdicate(ctx: Context<Abdicate>, heir_name: String) -> Result<()> {
        contexts::abdicate::abdicate(ctx, heir_name)
    }

    // Legislative Powers
    pub fn decree(ctx: Context<DecreeContext>, decree_text: String, decree_type: DecreeType) -> Result<()> {
        contexts::init_decree::decree(ctx, decree_text, decree_type)
    }

    pub fn repeal_decree(ctx: Context<RepealDecree>, decree_id: u64) -> Result<()> {
        contexts::init_decree::repeal_decree(ctx, decree_id)
    }

    pub fn appoint_official(ctx: Context<AppointOfficial>, role: String, jurisdiction: String) -> Result<()> {
        contexts::appoint::appoint_official(ctx, role, jurisdiction)
    }
    ...
    pub fn royal_judgment(ctx: Context<RoyalJudgment>, verdict: Verdict, punishment: Option<Punishment>) -> Result<()> {
        contexts::judicial::royal_judgment(ctx, verdict, punishment)
    }

    pub fn royal_pardon(ctx: Context<RoyalPardon>) -> Result<()> {
        contexts::judicial::royal_pardon(ctx)
    }

    pub fn declare_war(ctx: Context<DeclareWar>, reason: String) -> Result<()> {
        contexts::military::declare_war(ctx, reason)
    }
    ...
    pub fn set_economic_policy(
        ...
        contexts::set_economic_policy(ctx, policy, income_tax_rate, property_tax_rate, trade_tax_rate, luxury_tax_rate)
    }
    ...
    // pub fn form_alliance(ctx: Context<FormAlliance>, ally: Pubkey, terms: String) -> Result<()> {
    //     contexts::diplomacy::form_alliance(ctx, ally, terms)
    // }
